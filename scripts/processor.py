...
UA="Mozilla/5.0 (compatible; RegenMedCollector/0.4; +https://github.com/ri0115/saisei-collector)"
TIMEOUT=35
...
def find_row(plan, rows):
    ...
    for r in rows:
        ...
        if pf==rf: score+=0.12
        elif pf in rf or rf in pf: score+=0.05
        if pt==rt: score+=0.10
        if score>best_score:
...
def amount_candidates(text):
    ...
    for pidx,pat in enumerate(pats):
        for m in pat.finditer(t):
            raw=m.group(1)
            try:
                amount=int(float(raw.replace(",",""))*(10000 if pidx==1 else 1))
            except: continue
            if amount<1000 or amount>20000000: continue
            a=max(0,m.start()-140); b=min(len(t),m.end()+160)
            ctx=re.sub(r"\s+"," ",t[a:b]).strip()
            key=(amount,ctx)
            if key in seen: continue
            seen.add(key)
            score=0
            if re.search(r"治療費|施術料|料金|費用|価格|治療代|自己負担",ctx): score+=5
            if re.search(r"PRP|APS|ACP|GPS|幹細胞|再生医療|血小板",ctx,re.I): score+=2
            if re.search(r"税込|税別|税抜|消費税",ctx): score+=1
            if re.search(r"1回|一回|1部位|一部位|1施術|コース|回分",ctx): score+=1
            if re.search(r"キャンセル|返金|補償|賠償|損害|保険金|慰謝料",ctx): score-=8
            if re.search(r"検査料|診察料|初診料|再診料",ctx) and not re.search(r"治療費|施術料",ctx): score-=2
            tax="不明"
            if re.search(r"税込|消費税込",ctx): tax="税込"
            elif re.search(r"税別|税抜|消費税別",ctx): tax="税別"
            unit=""
            ...
            for token in ["ACP MAX","ACP","APS","GPS III","GPSⅢ","GPS","PEAK","Angel","TriCeLL","Mycells","Zimmer","Arthrex"]:
                if token.lower() in ctx.lower(): product.append(token)
            out.append({"amount":amount,"tax":tax,"unit":unit,"score":score,"products":product[:4],"excerpt":ctx})
...
def process_one(plan,indexes):
    ...
    try:
        ...
        if not row or score<0.62:
            result["status"]="FAIL"; result["error"]="MHLW index row not matched"
            return result
        ...
        for label,u in links:
            if "資料1" in label:
                doc=u; break
        if not doc and links: doc=links[0][1]
        if not doc:
            result["status"]="FAIL"; result["error"]="No explanation/consent document link"
            return result
        result["document_url"]=doc
        text,dtype=extract_document(doc)
        result["document_type"]=dtype
        prices=amount_candidates(text)
        result["prices"]=prices
        high=[p for p in prices if p["score"]>=5]
        if score>=0.84 and 1<=len(high)<=12:
            result["status"]="AUTO"
            result["prices"]=high
        elif prices:
            result["status"]="QC"
        else:
            result["status"]="QC"; result["error"]="Document fetched but no price candidate"
    except Exception as e:
        result["status"]="FAIL"; result["error"]=f"{type(e).__name__}: {e}"[:400]
    return result
...
def main():
    ...
    state.update({"version":"0.4","queue_version":queue.get("version"),"total":len(plans),"updated_at":now(),"phase":"loading_indexes"})
    save_json(args.state,state)

    if pending:
        indexes={"2":parse_index(2),"3":parse_index(3)}
        state["phase"]="processing"; state["index_counts"]={"2":len(indexes["2"]),"3":len(indexes["3"])}; save_json(args.state,state)
        with ThreadPoolExecutor(max_workers=max(1,args.workers)) as ex:
            futs={ex.submit(process_one,p,indexes):p["plan_id"] for p in pending}
            for fut in as_completed(futs):
                item=fut.result(); by_id[item["plan_id"]]=item
    ...
    latest={"version":"0.4","queue_version":queue.get("version"),"updated_at":now(),"summary":summary,"items":ordered}
    save_json(args.results,latest); write_csv(args.csv,ordered)
    state.update(summary); state["phase"]="complete" if summary["pending"]==0 else "checkpoint"; state["updated_at"]=now()
    save_json(args.state,state)
