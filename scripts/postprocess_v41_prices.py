...
NO_PUBLIC_PRICE=Path("data/v41_no_public_price_overrides.json")
...
def main():
    ...
    no_public_price={}
    if NO_PUBLIC_PRICE.exists():
        no_public_price=(json.loads(NO_PUBLIC_PRICE.read_text(encoding="utf-8")).get("plans") or {})
    promoted=0
    override_promoted=0
    normalized_auto=0
    normalized_qc=0
    ...
    for item in items:
        ...
        np=no_public_price.get(item.get("plan_id"))
        if sx:
            ...
            if np and item.get("status_normalized")!="AUTO":
                item["status_normalized"]="NO_PUBLIC_PRICE"
                item["postprocess_resolution"]="verified_no_public_price"
                item["public_price_status"]=np.get("public_price_status","CONTACT_REQUIRED")
                item["verified_source_url"]=np.get("source_url","")
                item["verified_source_type"]=np.get("source_type","")
                item["no_public_price_note"]=np.get("note","")

        status=item["status_normalized"]
        if status=="AUTO": normalized_auto+=1
        elif status=="QC": normalized_qc+=1
        elif status=="NO_PUBLIC_PRICE": normalized_no_public_price+=1
    ...
    summary={
        ...
        "normalized_no_public_price":normalized_no_public_price,
        "in_scope_total":len(items)-normalized_out_of_scope,
        "resolved_in_scope":normalized_auto+normalized_no_public_price,
