...
ANC_RE=re.compile(r"キャンセル料|初診料|再診料|診察料|検査料|血液検査|感染症検査|採血料|保管料|保存料|文書料|証明書|処置費用|装置.*費用|手数料",re.I)
TREAT_RE=re.compile(r"治療費(?:用)?|施術料|治療料金|投与費用|培養費用|PRP|APS|ACP|GPS|Angel|Condensia|コンデンシア|PRGF|Endoret|幹細胞培養|細胞投与|1部位|1回",re.I)
COMPLEX_FEE_RE=re.compile(r"治療費とは別|別途|初診料|再診料|診察料|検査料|検査費用|キャンセル料|保管料|保管費用|採取料|処置費用|手数料",re.I)
...
def resolve_qc_item(item,normed):
    kept=[p for p in normed if p.get("postprocess_keep")]
    uniq=[];seen=set()
    for p in kept:
    ...
    if any(RANGE_RE.search(p.get("line") or "") for p in kept):
        return False,kept,"price_range_needs_review"

    alltext=" ".join((p.get("excerpt") or "")+" "+(p.get("line") or "") for p in normed)
    ...
    if len(kept)==1:
        p=kept[0]
        if stem and p.get("amount",0)<500000:
            return False,kept,"stem_cell_component_needs_review"
        if p.get("postprocess_reason") in ("paired_product_price","cell_dose_table","explicit_treatment_price","explicit_prp_product_line"):
            return True,kept,"single_clear_treatment_price"

    if complex_fee:
        return False,kept,"multiple_fee_components_needs_review"

    if 2<=len(kept)<=10 and all(p.get("product_pair_method")=="nearest_preceding_product_in_excerpt" for p in kept):
        return True,kept,"structured_product_price_table"

    if stem and 2<=len(kept)<=10 and all(p.get("postprocess_reason")=="cell_dose_table" and p.get("amount",0)>=500000 for p in kept):
...
def main():
    ...
    for item in items:
        ...
        normed=[normalize_candidate(p,item.get("treatment_class","")) for p in (item.get("prices") or [])]
    ...
    OUT_JSON.write_text(json.dumps({"version":"v41-postprocess-1.0","updated_at":now(),"summary":summary,"items":items},ensure_ascii=False,indent=2),encoding="utf-8")
    OUT_SUM.write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding="utf-8")

    cols=["plan_id","mhlw_plan_code","v41_lane","facility_id","prefecture","facility","category","treatment","treatment_class","status_source","status_normalized","postprocess_resolution","document_url","amount","tax","unit","products","makers","product_pair_method","postprocess_reason","score","page","excerpt"]
        ...
        for it in items:
            for p in (it.get("prices_normalized") or [None]):
                w.writerow({
                    ...
                    "unit":"" if not p else p.get("unit",""),"products":"" if not p else "|".join(p.get("products") or []),
